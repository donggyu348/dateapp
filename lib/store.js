...
const COLLECTIONS = ['users', 'sessions', 'contests', 'saves', 'plans', 'analyses'];
...
async function createPgStore(url) {
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS specfit_records (
  ...
  const { rows } = await pool.query('SELECT collection, id, data FROM specfit_records');
  for (const r of rows) {
  ...
  async function writeChanges() {
    ...
    try {
      ...
      if (up.id.length) {
        await client.query(
          `INSERT INTO specfit_records (collection, id, data)
      ...
      if (del.length) {
        await client.query(
          `DELETE FROM specfit_records WHERE (collection, id) IN (SELECT * FROM unnest($1::text[], $2::text[]))`,
